...
export class AuthController {
  ...
  @Post("login")
  async authLogin(@Body() loginDto: LoginDto, @Req() req: Request) {
    return await this.authService.login(loginDto, req);
  }

  // @UseGuards(RecaptchaV3Guard)
  @Post("register")
  async authRegister(@Body() registerDto: RegisterDto, @Req() req: Request) {
    return await this.authService.register(registerDto, req);
  }

  @UseGuards(RecaptchaV3Guard)
  @Post("oauth")
  async authOauth(@Body() body: OAuthDto, @Req() req: Request) {
    if (!body?.code || !body?.provider) {
      throw new BadRequestException("Invalid code or provider");
    }
    switch (body?.provider) {
      case "google":
        return await this.authService.googleOAuth(body?.code, req);
      case "github":
        return await this.authService.githubOAuth(body?.code, req);
      default:
  ...
  async authLogout(
    ...
    res.clearCookie(
      this.configService.get<string>("USER_TOKEN_COOKIE_NAME") || "tdt_t"
    );
