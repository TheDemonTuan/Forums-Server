...
export class CacheService {
  ...
  public async setUserToken(uuid: string, userToken: UserToken) {
    ...
    return this.cacheManager.set(
      this.utCacheName + uuid,
      {
        ...
        [userToken?.id]: {
          ...userToken,
          expired: new Date().getTime() + this.utCacheTime,
        },
      },
      this.utCacheTime
    );
  ...
  public async getUserTokens(uuid: string) {
    ...
    for (const key in userTokens)
      userTokens.hasOwnProperty(key) && userTokens[key]?.expired < new Date().getTime() && delete userTokens[key];
